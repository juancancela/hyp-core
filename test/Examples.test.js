...
var ShoppingCartMachineWithMutableStates = require('../examples/ShoppingCartMachine').ShoppingCartMachineWithMutableStates;
...
describe('Examples', function () {
    ...
    var machineWithMutableState = new ShoppingCartMachineWithMutableStates();
    ...
    it('should successfully construct a simple shopping cart machine', function () {
        ...
        assert.equal(Object.keys(machine.getProperties())[0], "items", "Item property must be defined for machine");
    ...
    it('should be able to transition from NON_PRICEABLE to PRICEABLE through the addItem Operation', function () {
        machine.run("addItem", null, function (err, res) {
            ...
            assert.equal("Argentina->Brasil", machine.getProperties()["items"].getValue()[0], "Items machine property was sucessfully updated");
    ...
    it('should be able to create a machine that contains a transition where final state mutates', function(){
        machineWithMutableState.getProperties()["items"].setValue([new CartItem("Argentina", "Brasil"), new CartItem("Venezuela", "Paraguay")]);
